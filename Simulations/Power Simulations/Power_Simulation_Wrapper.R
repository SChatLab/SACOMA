...
qual.regions <- get.GenoregsCorr(minCpgs.regions, 
                                 ...
                                 maxCorr = 1,
                                 numcores = numcores)
all.tags <- names(minCpgs.regions)
wh.tags <- all.tags[which(!all.tags %in% names(qual.regions))]
unqual.regions <- minCpgs.regions[wh.tags]
sigRegs <- length(qual.regions)
noiseRegs <- fold*length(qual.regions)
...
exports <- c('Sacoma', 'trigger.coMethDMR', 'trigger.Aclust2', 'perfMetrics', 
             'qual.regions', 'unqual.regions', 'trigger.CoMeBack', 'Sacoma.fast')
restmp <- foreach(r = 1:nreps, .combine = 'rbind', .packages = packages, .export = exports) %dopar% {
  set.seed(1233 + r)
  unqual.regions <- unqual.regions[sample(names(unqual.regions), noiseRegs)]
  cpgLocs1 <- do.call('rbind', qual.regions)
  cpgLocs2 <- do.call('rbind', unqual.regions)
  cpgLocs2$cor.stats <- 0.3
  cpgLocs2$Truth <- 'noise'
  cpgLocs <- data.frame(rbind(cpgLocs1, cpgLocs2))
  ...
  restmp <- perfMetrics(Method = Method,
                        all.cpgs = cpgLocs,
                        predictions = fit,
                        nrep = r)
...
sim.scene <- paste(Method, data, datatype, minCorr, minCpgs, maxGap, fold, sep = '_')
savenam <- paste(sim.scene, ".RData", sep = '')
save(restmp, file = paste(dir, "Results", savenam, sep = '/'))